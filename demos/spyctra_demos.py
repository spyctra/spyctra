...
def get_point_demo():
    ...
    for i in range(trials):
        a.add(fake_spyctra(amp=amps_in[i], noise=4, points=4096, delta=1, t_2=100))
...
def get_snr_demo():
    ...
    for i in range(trials):
        a.add(fake_spyctra(df=i*16, phi=i, noise=noises_in[i], t_2=1e-2))
    ...
    snrs = a.get_snr()
    snrs_at_x_0 = a.get_snr(a.points//2-1)
    snrs_at_x_i = a.get_snr(a.points//2-1 + np.arange(a.count))
...
def integrate_demo():
    ...
    plt.plot(amps, integrals, label='integrals')
...
def new_count_more_demo():
    ...
    a.plot()
    savefig('new_count_more_pre.png')
    a.new_count(4)
    ...
    a.plot()
    savefig('new_count_more_post.png')
    show()
...
def new_count_less_demo():
    ...
    trials = 8
    amps = np.logspace(3,2,trials)

    for i in range(trials):
        a.add(fake_spyctra(amp=amps[i], noise=16, points=256, t_2=2e-2))

    a.plot()
    savefig('new_count_less_pre.png')
    show()

    a.new_count(1)

    a.plot()
    savefig('new_count_less_post.png')
    show()
...
def sort_demo():
    ...
    for i in range(trials):
        a.add(fake_spyctra(df=dfs[i], t_2=3e-3, noise=16, points=1024))

    a.fft()
    a.resize([-1000,1000])

    a.plot()
    savefig('sort_pre.png')
    show()

    a.sort(dfs)

    a.plot()
    savefig('sort_post.png')
    show()
