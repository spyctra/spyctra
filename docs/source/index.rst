...
Spyctra objects manipulate the following attributes:
...
* Why lists and not 2D numpy arrays to store data? Because *self.data* is a list it is much easier to add or remove spyctra from *self*. Especially when data is streaming in, it may not be clear how much data you have to process and the performance hit from forcing 2D numpy arrays can be obnoxious.
...
* Why is phi used as the variable for phase? Regarding the spyctra object, phase is a verb and phi is a noun. However, through the documentation I refer to the tracked values of phi as phase.
